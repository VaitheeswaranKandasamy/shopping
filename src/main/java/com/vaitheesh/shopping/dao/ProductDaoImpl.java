...
public class ProductDaoImpl implements ProductDao {
	...
	public Product findBy(Long idProduct) {
		...
		return (Product) getSession().createQuery(queryString).setParameter("idProduct", idProduct).uniqueResult();
	}

	@Override
	public Product findBy(String description) {
		String queryString = "SELECT p FROM Product AS p WHERE p.description = :description";
		return (Product) getSession().createQuery(queryString).setParameter("description", description).uniqueResult();
	}
	...
	public List<Product> findByCategory(String category) {
		...
		return getSession().createQuery(queryString).setParameter("category", category).list();
	}
